...
const API_KEY = 'L3OMwNgR_KHg1IVCDhCSmQ'; // Replace with your actual API key
...
async function createTrack(prompt) {
    try {
        ...
        return response.data.tracks[0]; // Returns the track ID
    } catch (error) {
...
async function composeTrack(trackId) {
    try {
        ...
        return response.data.task_id; // Returns the task ID
    } catch (error) {
...
async function checkCompositionStatus(taskId) {
    try {
        ...
        return response.data; // Returns status and track URL when composed
    } catch (error) {
